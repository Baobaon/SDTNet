...
from models.Ghost import Ghost_conv
...
class Changer_channel_exchange(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # exchange_mask1 = exchange_mask.int().expand((N, C)).unsqueeze(-1).unsqueeze(-1)  # b,c,1,1
...
class DPFA(nn.Module):
    ...
    def forward(self, t1, t2, log=None, module_name=None,
        ...
        fuse = stack_attention[1] * t2 + stack_attention[0] * t1   # b,c,h,w
...
class Encoder_lastBlock(nn.Module):
    """Basic block in decoder."""
...
# 可视化黑白图
def log_feature_2(log_list, module_name, feature_name_list, img_name, module_output=True, label=None):
...
def replace_zero_neighbors(tensor):
    # 获取张量的形状
        ...
        for j in range(channels):
            for h in range(height):
                for w in range(width):
                    if tensor[i,j,h,w] != 0 and h!=0 and w !=0 and h!=255 and w!=255:
                        if tensor[i,j,h,w-1] == 0 and tensor[i,j,h,w+1] == 0 and tensor[i,j,h-1,w] == 0 and tensor[i,j,h+1,w] == 0:
                            tensor[i,j,h,w] = 0
    ...
    for k, log in enumerate(log_list):
        ...
        if module_output:
            ...
            for i in range(b):
                ...
                change_map.save(log_dir + img_name[i] + '.jpg')
                # log_i.save(log_dir + img_name[i] + '.jpg')